let apiArticleApi = new TempApi.ArticleApi();import TempApi from '../src/index';let article = new TempApi.Article();document.getElementById('i974').onclick = (event) => {
...
$(
    ...
    );document.getElementById('i1umx').onclick = (event) => {
    event.preventDefault();
    article['articleImage'] = {
      ...
      article['articledate'] = document.querySelector("[annotationname = 'articledate']").value.replace(/(?<day>\d+)-(?<month>\d+-)/,'$<month>$<day>-');article['articletitle'] = document.querySelector("[annotationname = 'articletitle']").value;article['articletext'] = document.querySelector("[annotationname = 'articletext']").value;apiArticleApi.createarticle( article, (error, data, response) => { if (error) {console.error(error);} else { console.log('API called successfully. Returned data: ' + data); {  location.href= '/homePage/'+response.body.query._id+'' ;}}});};window.onload = () => {};